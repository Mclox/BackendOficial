...
class RoleModel {
    ...
    static async create(data) {
        const { nombre, descripcion, estado, permisos } = data;
        const permisosFinal = JSON.stringify(permisos || []);

        const query = `
            INSERT INTO Roles (nombre, descripcion, estado, permisos)
            VALUES ($1, $2, $3, $4)
            RETURNING id_rol
        `;
        
        const result = await db.query(query, [nombre, descripcion, estado || 'Activo', permisosFinal]);
    ...
    static async update(id, data) {
        ...
        const query = `
            UPDATE Roles 
            SET nombre = $1, descripcion = $2, estado = $3, permisos = $4
            WHERE id_rol = $5
        `;
        const result = await db.query(query, [nombre, descripcion, estado, permisosFinal, id]);
        return result.rowCount > 0;
    }

    static async delete(id) {
        const result = await db.query('DELETE FROM Roles WHERE id_rol = $1', [id]);
        return result.rowCount > 0;
    }
}
